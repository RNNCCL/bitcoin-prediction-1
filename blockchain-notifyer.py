...
class Price:
	...
	def priceHistorical(self, from_curr, to_curr, ts, markets=False, e=None, extraParams=None,
	...
	def __get_price(self, baseurl, from_curr, to_curr, e=None, extraParams=None, sign=False,
		...
		if len(args) >= 2:
			return self.__get_url(baseurl + '&'.join(args)), args
		else:
...
afef = Price().get_url("https://min-api.cryptocompare.com/data/pricehistorical?fsym=ETH&tsyms=BTC,USD,EUR&ts=1452680400")
for i in afef.values():
	if isinstance(afef, unicode):
